...
from .aws import AwsFacade, PreflightError
...
def validate(config: Path = typer.Option(..., exists=True, readable=True)) -> None:
    ...
    cfg = _config(config)
    try:
        targets = AwsFacade(cfg.aws_profile).preflight(cfg)
    except PreflightError as exc:
        console.print(f"[red]Preflight failed:[/red] {exc}")
        raise typer.Exit(2) from exc
    table = Table("Location", "Region", "Zone", "Type", "Opt-in")
...
def deploy(
    ...
    yes: bool = typer.Option(
        False,
        "--yes",
        help="Skip the pause that confirms each EIP is registered in Cisco Secure Access.",
    ),
) -> None:
